...
class DecayMeasure:
    def __init__(self):
        # SERIAL_NUM = 'CS962/010'
        # self.ps = ps5000a.PS5000a(serialNumber=SERIAL_NUM, connect=False)
    ...
    def openScope(self):
        ...
        self.ps.setChannel("A", coupling="DC", VRange=20.0E-3, VOffset=-3.0E-3, enabled=True)
    ...
    def accumulate(self, sweep_no):
        ...
        title = time.strftime("%d%m%y_%H%M%S", time.localtime())

        ################## MARK #####################################################
        finger = 'nf'
        #############################################################################

        fname = "Data\\device3\\" + title + '_' + finger + '_' + '.txt'
